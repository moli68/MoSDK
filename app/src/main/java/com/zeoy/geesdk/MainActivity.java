...
public class MainActivity extends AppCompatActivity {
    ...
    public void pay(int pid) {
        ...
        if (Utils.isNetworkAvailable(this)) {
            HttpUtils.getInstance().postOrder(1, pid, 0, 2, new BaseCallback<MoOrderResultBean>() {
                @Override
                ...
                @Override
                public void onSuccess(Response response, MoOrderResultBean apliyBean) {
                    if (response.isSuccessful()) {
                        if (apliyBean != null) {
                            ...
                            Runnable runnable = () -> {
                                PayTask alipay = new PayTask(MainActivity.this);
                                Map<String, String> map = alipay.payV2(apliyBean.getPackageX(), true);
                                Message msg = new Message();
                                msg.what = SDK_PAY_FLAG;
                                msg.obj = map;
                                mHandler.sendMessage(msg);
